  ...
  Assignments.assignTaskToUser(req.body.id, req.params.taskId, req.user)
    ...
    .catch(function (response) {
      if (response == 403) {
        ...
        utils.writeJson(
          res,
          { errors: [{ param: "Server", msg: `Task ${req.params.taskId} already assigned to this user` }] },
          404
        );
      } else {
        utils.writeJson(
        ...
        utils.writeJson(
          res,
          {
            errors: [
              {
                param: "Server",
                msg: "The user is not an assignee of the task or the task is active for another user.",
              },
            ],
          },
          403
