...
module.exports.getPublicTasks = function getPublicTasks(req, res, next) {
  ...
  Tasks.getPublicTasksTotal().then((response) => {
    numOfTasks = response;
    let totalPage = Math.ceil(numOfTasks / constants.OFFSET);
    if (!(numOfTasks > 0 && totalPage > 0)) {
      utils.writeJson(
        res,
        {
          errors: [{ param: "Server", msg: "There aren't public Tasks (yet)" }],
        },
        404
      );
    } else {
...
module.exports.getOwnedTasks = function getUserTasks(req, res, next) {
  ...
  var numOfTasks;
  Tasks.getOwnedTasksTotal(req).then((response) => {
    numOfTasks = response;
    Tasks.getOwnedTasks(req).then((response) => {
...
module.exports.getAssignedTasks = function getAssignedTasks(req, res, next) {
  ...
  var numOfTasks;
  Tasks.getAssignedTasksTotal(req).then((response) => {
    numOfTasks = response;
    Tasks.getAssignedTasks(req).then((response) => {
      if (req.query.pageNo == null) var pageNo = 1;
      else var pageNo = req.query.pageNo;
      var totalPage = Math.ceil(numOfTasks / constants.OFFSET);
