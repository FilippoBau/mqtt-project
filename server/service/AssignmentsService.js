  ...
  return new Promise((resolve, reject) => {
    ...
    db.all(sql1, [taskId], (err, rows) => {
      if (err) reject(err);
        ...
        const sql2 = "SELECT user FROM assignments a WHERE a.task = ?";
        db.all(sql2, [taskId], (err, user) => {
          if (err) reject(err);
          else if (
            user.find((u) => {
            ...
            const sql3 = "INSERT INTO assignments(task, user) VALUES(?,?)";
            db.run(sql3, [taskId, userId], function (err) {
              if (err) {
                reject(err);
              } else {
                resolve(null);
    ...
    db.serialize(function () {
      ...
      db.all(sql1, [taskId], function (err, check) {
        if (err) {
          ...
          // Check if exists a Task where the user is already active
          const sql2 =
          ...
          db.all(sql2, [userId], function (err, rows1) {
            if (err) {
              ...
              var deselected = null;
              // Save the task ID to be deselected
              if (rows1.length !== 0) deselected = rows1[0].id;
              // Get deselected task info
              const sql3 =
              ...
              db.all(sql3, [userId, taskId], function (err, rows2) {
                if (err) {
                  ...
                  // Update table deselecting all Tasks active for that user
                  const sql4 =
                  ...
                  db.run(sql4, [userId], function (err) {
                    if (err) {
                      ...
                      // Update Task to Active for that User if there aren't other users avtive on the task
                      const sql5 =
                      ...
                      db.run(
                        ...
                        function (err) {
                          if (err) {
                            ...
                            var message = new MQTTTaskMessage(
                              "active",
                              parseInt(userId),
                              rows2[0].name
                            );
